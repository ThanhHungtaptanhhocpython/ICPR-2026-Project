#!/usr/bin/env python3
"""Automation script to run ablation experiments for STN impact analysis."""
...
def build_command(experiment_config: Dict[str, Any], output_dir: str = "experiments") -> List[str]:
    ...
    # Handle extra flags such as --use-stn.
...
def _parse_best_accuracy(log_path: str) -> Optional[float]:
    ...
    try:
        with open(log_path, "r") as f:
            for line in f:
                line = line.strip()
                for pattern in ["Best validation accuracy:", "Best Val Acc:", "Best accuracy:"]:
                    if pattern in line:
                        try:
...
def main() -> None:
    ...
    # The Kaggle-safe default disables STN; pass "--use-stn" to enable it.
